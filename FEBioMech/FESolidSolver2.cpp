...
void FESolidSolver2::UpdateKinematics(vector<double>& ui)
{
	...
	for (i=0; i<mesh.Nodes(); ++i)
	{
		...
		if ((n = node.m_ID[DOF_X]) >= 0) node.set(DOF_X, ui[n] + m_Ui[n] + m_Ut[n]);
		if ((n = node.m_ID[DOF_Y]) >= 0) node.set(DOF_Y, ui[n] + m_Ui[n] + m_Ut[n]);
		if ((n = node.m_ID[DOF_Z]) >= 0) node.set(DOF_Z, ui[n] + m_Ui[n] + m_Ut[n]);
