...
void FESolidSolver2::UpdateKinematics(vector<double>& ui)
{
	...
	for (i=0; i<mesh.Nodes(); ++i)
	{
		...
		if ((n = node.m_ID[m_dofX]) >= 0) node.set(m_dofX, ui[n] + m_Ui[n] + m_Ut[n]);
		if ((n = node.m_ID[m_dofY]) >= 0) node.set(m_dofY, ui[n] + m_Ui[n] + m_Ut[n]);
		if ((n = node.m_ID[m_dofZ]) >= 0) node.set(m_dofZ, ui[n] + m_Ui[n] + m_Ut[n]);
