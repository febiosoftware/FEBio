...
bool FEM::InitPoro()
{
	...
	for (i=0; i<Materials(); ++i)
		if ((dynamic_cast<FEPoroElastic*>(m_MAT[i])) || (dynamic_cast<FEBiphasic*>(m_MAT[i])))
		{
	...
	if (m_bsym_poro == false) 
	{
		...
		if (m_pStep->m_psolver->m_bfgs.m_maxups != 0)
		{
			...
			clog.printbox("WARNING", "The non-symmetric biphasic algorithm does not work with BFGS yet.\nThe full-Newton method will be used instead.");
		}
	}
