...
//-----------------------------------------------------------------------------
FETriphasicSolver::FETriphasicSolver(FEModel& fem) : FEBiphasicSolver(fem)
{
	m_Ctol = 0.01;
}
...
*/
	...
	{
		// calculate the internal fluid work
		for (i=0; i<mesh.Domains(); ++i) 
		{
		...
		// calculate the internal solute work for solute 0
		for (i=0; i<mesh.Domains(); ++i) 
		{
			FETriphasicDomain& dom = dynamic_cast<FETriphasicDomain&>(mesh.Domain(i));
			dom.InternalSoluteWorkSS(this, R, dt, 0);
		}

		// calculate the internal solute work for solute 1
		for (i=0; i<mesh.Domains(); ++i) 
		{
			FETriphasicDomain& dom = dynamic_cast<FETriphasicDomain&>(mesh.Domain(i));
			dom.InternalSoluteWorkSS(this, R, dt, 1);
		}
	}
	else
	{
		...
		{
			FETriphasicDomain& dom = dynamic_cast<FETriphasicDomain&>(mesh.Domain(i));
			dom.InternalSoluteWork(this, R, dt, 0);
		}
		...
		{
			FETriphasicDomain& dom = dynamic_cast<FETriphasicDomain&>(mesh.Domain(i));
			dom.InternalSoluteWork(this, R, dt, 1);
		}
	}
...
bool FETriphasicSolver::StiffnessMatrix()
{
