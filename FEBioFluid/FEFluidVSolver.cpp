...
BEGIN_FECORE_CLASS(FEFluidVSolver, FENewtonSolver)
...
//-----------------------------------------------------------------------------
//! FEFluidVSolver Construction
//
FEFluidVSolver::FEFluidVSolver(FEModel* pfem) : FENewtonSolver(pfem)
{
    // default values
    m_Rtol = 0.001;
    m_Etol = 0.01;
...
//-----------------------------------------------------------------------------
FEFluidVSolver::~FEFluidVSolver()
{
    
}

//-----------------------------------------------------------------------------
//! Allocates and initializes the data structures used by the FEFluidVSolver
//
bool FEFluidVSolver::Init()
{
...
bool FEFluidVSolver::InitEquations()
{
    // base class initialization
    FENewtonSolver::InitEquations();
    
    // determined the nr of velocity equations
    FEMesh& mesh = GetFEModel()->GetMesh();
    m_nveq = 0;
...
//! accelerations, etc.
void FEFluidVSolver::UpdateKinematics(vector<double>& ui)
{
    // get the mesh
...
void FEFluidVSolver::Update(vector<double>& ui)
{
    TRACK_TIME("update");
...
//-----------------------------------------------------------------------------
bool FEFluidVSolver::InitStep(double time)
{
    FEModel& fem = *GetFEModel();
...
bool FEFluidVSolver::StiffnessMatrix()
{
    FEModel& fem = *GetFEModel();
...
void FEFluidVSolver::NonLinearConstraintStiffness(const FETimeInfo& tp)
{
    FEModel& fem = *GetFEModel();
...
void FEFluidVSolver::ContactStiffness()
{
    FEModel& fem = *GetFEModel();
...
//-----------------------------------------------------------------------------
void FEFluidVSolver::AssembleResidual(int node_id, int dof, double f, vector<double>& R)
{
    FEModel& fem = *GetFEModel();
...
void FEFluidVSolver::AssembleStiffness(std::vector<int>& lm, matrix& ke)
{
    m_pK->Assemble(ke, lm);
}
...
void FEFluidVSolver::AssembleStiffness(vector<int>& en, vector<int>& elm, matrix& ke)
{
    FEModel& fem = *GetFEModel();
...
void FEFluidVSolver::ContactForces(FEGlobalVector& R)
{
    FEModel& fem = *GetFEModel();
...
bool FEFluidVSolver::Residual(vector<double>& R)
{
    TRACK_TIME("residual");
...
void FEFluidVSolver::NonLinearConstraintForces(FEGlobalVector& R, const FETimeInfo& tp)
{
    FEModel& fem = *GetFEModel();
...
void FEFluidVSolver::NodalForces(vector<double>& F, const FETimeInfo& tp)
{
