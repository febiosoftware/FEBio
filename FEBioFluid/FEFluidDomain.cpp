...
FEFluidDomain::FEFluidDomain(FEModel* pfem) : FESolidDomain(&pfem->GetMesh())
{
    m_pMat = 0;
    m_btrans = true;
}
...
FEFluidDomain& FEFluidDomain::operator = (FEFluidDomain& d)
{
    m_Elem = d.m_Elem;
    m_pMesh = d.m_pMesh;
    return (*this);
...
//-----------------------------------------------------------------------------
//! Assign material
void FEFluidDomain::SetMaterial(FEMaterial* pmat)
{
    if (pmat)
    {
...
FEDomain* FEFluidDomain::Copy()
{
    FEFluidDomain* pd = new FEFluidDomain(0);
    pd->m_Elem = m_Elem;
    pd->m_Node = m_Node;
    return pd;
}
...
bool FEFluidDomain::Initialize(FEModel &fem)
{
...
void FEFluidDomain::InitElements()
{
...
//-----------------------------------------------------------------------------
void FEFluidDomain::InternalForces(FEGlobalVector& R)
{
...
void FEFluidDomain::ElementInternalForce(FESolidElement& el, vector<double>& fe)
{
    int i, n;
...
//-----------------------------------------------------------------------------
void FEFluidDomain::BodyForce(FEGlobalVector& R, FEBodyForce& BF)
{
    int NE = (int)m_Elem.size();
...
void FEFluidDomain::ElementBodyForce(FEBodyForce& BF, FESolidElement& el, vector<double>& fe)
{
    // jacobian
    double detJ;
    double *H;
...
void FEFluidDomain::ElementBodyForceStiffness(FEBodyForce& BF, FESolidElement &el, matrix &ke)
{
    int neln = el.Nodes();
...
void FEFluidDomain::ElementMaterialStiffness(FESolidElement &el, matrix &ke)
{
    int i, i4, j, j4, n;
...
//-----------------------------------------------------------------------------
void FEFluidDomain::StiffnessMatrix(FESolver* psolver)
{
...
//-----------------------------------------------------------------------------
void FEFluidDomain::MassMatrix(FESolver* psolver)
{
    // repeat over all solid elements
    int NE = (int)m_Elem.size();
...
//-----------------------------------------------------------------------------
void FEFluidDomain::BodyForceStiffness(FESolver* psolver, FEBodyForce& bf)
{
...
void FEFluidDomain::ElementStiffness(FEModel& fem, int iel, matrix& ke)
{
...
void FEFluidDomain::ElementMassMatrix(FESolidElement& el, matrix& ke)
{
    int i, i4, j, j4, n;
...
//-----------------------------------------------------------------------------
void FEFluidDomain::UpdateStresses(FEModel &fem)
{
...
void FEFluidDomain::UpdateElementStress(int iel, double dt)
{
...
void FEFluidDomain::UnpackLM(FEElement& el, vector<int>& lm)
{
    int N = el.Nodes();
    lm.resize(N*4);
    for (int i=0; i<N; ++i)
...
//-----------------------------------------------------------------------------
void FEFluidDomain::InertialForces(FEGlobalVector& R)
{
    int NE = (int)m_Elem.size();
#pragma omp parallel for shared (NE)
    for (int i=0; i<NE; ++i)
...
void FEFluidDomain::ElementInertialForce(FESolidElement& el, vector<double>& fe)
{
    int i, n;
