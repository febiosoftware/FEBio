...
BEGIN_FECORE_CLASS(FEBackFlowVStabilization, FESurfaceLoad)
ADD_PARAMETER(m_beta, "beta");
END_FECORE_CLASS()

//-----------------------------------------------------------------------------
//! constructor
FEBackFlowVStabilization::FEBackFlowVStabilization(FEModel* pfem) : FESurfaceLoad(pfem)
{
    m_beta = 1.0;
    m_rho = 1.0;
...
//-----------------------------------------------------------------------------
//! allocate storage
void FEBackFlowVStabilization::SetSurface(FESurface* ps)
{
...
//-----------------------------------------------------------------------------
//! initialize
bool FEBackFlowVStabilization::Init()
{
...
void FEBackFlowVStabilization::Serialize(DumpStream& ar)
{
...
void FEBackFlowVStabilization::ElementStiffness(FESurfaceElement& el, matrix& ke, const FETimeInfo& tp)
{
    ...
    // solid element associated with this surface element
    FESolidElement& se = static_cast<FESolidElement&>(*el.m_elem[0]);
    // get first material point of that element and get the fluid pressure
    FEMaterialPoint& mp = *se.GetMaterialPoint(0);
    FEFluidVMaterialPoint& vpt = *(mp.ExtractData<FEFluidVMaterialPoint>());
    double dp = 0;
    if (vpt.m_kJJ != 0) dp = vpt.m_dpf/vpt.m_kJJ;
    vector<vec3d> kJv = vpt.m_kJv;
    for (int i=0; i<kJv.size(); ++i) kJv[i] *= dp;
    ...
    for (int k=0; k<nint; ++k)
    {
        ...
        if (m_beta*vn < 0) {
            ...
            for (int i=0; i<neln; ++i)
                for (int j=0; j<neln; ++j)
                {
                    mat3d Kdp = (n & kJv[j])*(da*w[k]);
                    mat3d Kww = (K*(N[i]*N[j]) + Kdp*N[i])*tp.alphaf;
...
void FEBackFlowVStabilization::ElementForce(FESurfaceElement& el, vector<double>& fe, const FETimeInfo& tp)
{
    ...
    FEFluidVMaterialPoint& vpt = *(mp.ExtractData<FEFluidVMaterialPoint>());
    double p = pt.m_pf;
    if (vpt.m_kJJ != 0) p -= vpt.m_fJ*vpt.m_dpf/vpt.m_kJJ;
    ...
    for (int j=0; j<nint; ++j)
    {
        ...
        if (m_beta*vn < 0) {
            vec3d f = n*((-p+m_beta*m_rho*vn*vn)*da*w[j]);
...
//-----------------------------------------------------------------------------
void FEBackFlowVStabilization::StiffnessMatrix(const FETimeInfo& tp, FESolver* psolver)
{
...
//-----------------------------------------------------------------------------
void FEBackFlowVStabilization::Residual(const FETimeInfo& tp, FEGlobalVector& R)
{
    FESurface& surf = GetSurface();
    int npr = surf.Elements();
#pragma omp parallel for
...
//-----------------------------------------------------------------------------
void FEBackFlowVStabilization::UnpackLM(FEElement& el, vector<int>& lm)
{
