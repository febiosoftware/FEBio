...
void FETet20_::shape_deriv(double* Hr, double* Hs, double* Ht, double r, double s, double t)
{
	...
	Hr[ 0] = -2.0*(3*L1 - 2)*L1 - 5.0/2.0*(3*L1 - 1)*L1 - 0.5*(3*L1 - 1)*(3*L1 - 2);
	Hr[ 1] = (3*L2 - 2)*L2 + 0.5*(3*L2 - 1)*L2 + 0.5*(3*L2 - 1)*(3*L2 - 2);
	Hr[ 2] = -0.5*(3*L3 - 2)*L3 - (3*L3 - 1)*L3;
	Hr[ 3] = -0.5*(3*L4 - 2)*L4 - (3*L4 - 1)*L4;
	Hr[ 4] = -18.0*L1*L2 - 9.0/2.0*(3*L1 - 1)*L2 + 9.0/2.0*(3*L1 - 1)*L1;
	Hr[ 5] = 9*L1*L2 - 9.0/2.0*(3*L2 - 1)*L2 + 9.0/2.0*(3*L2 - 1)*L1;
	Hr[ 6] = 9*L2*L3 + 9.0/2.0*(3*L2 - 1)*L3;
	Hr[ 7] = -9.0/2.0*L2*L3 + 9.0/2.0*(3*L3-1)*L3;
	Hr[ 8] = -18*L1*L3-9.0/2.0*(3*L1-1)*L3;
	Hr[ 9] = -9./2.*L1*L3-9.0/2.0*(3*L3-1)*L3;
	Hr[10] = -18*L1*L4-9.0/2.0*(3*L1-1)*L4;
	Hr[11] = -9./2.*L1*L4-9./2.*(3*L4-1)*L4;
	Hr[12] = 9*L2*L4 + 9./2.*(3*L2-1)*L4;
	Hr[13] = -9./2.*L2*L4+9./2.*(3*L4-1)*L4;
	Hr[14] = -9./2.*L3*L4;
	Hr[15] = -9./2.*L3*L4;
	...
	Hs[ 0] = -2.*(3*L1-2)*L1 - 5./2.*(3*L1 - 1)*L1 - 0.5*(3*L1 - 1)*(3*L1 - 2);
	Hs[ 1] = -0.5*(3*L2 - 2)*L2 - (3*L2 - 1)*L2;
	Hs[ 2] = (3*L3 - 2)*L3 + 0.5*(3*L3-1)*L3 + 0.5*(3*L3 - 1)*(3*L3 - 2);
	Hs[ 3] = -0.5*(3*L4 - 2)*L4 - (3*L4 - 1)*(3*L4 - 2)*L4;
	Hs[ 4] = -9*L1*L2 - 9./2.*(3*L1 - 1)*L2;
	Hs[ 5] = -9./2.*L1*L2 - 9./2.*(3*L2 - 1)*L2;
	Hs[ 6] = -9./2.*L2*L3 + 9./2.*(3*L2 - 1)*L2;
	Hs[ 7] = 9*L2*L3 + 9./2.*(3*L3-1)*L2;
	Hs[ 8] = -9*L1*L3 - 9./2.*(3*L1 - 1)*L3 + 9./2.*(3*L1 - 1)*L1;
	Hs[ 9] = 9*L1*L3 - 9./2.*(3*L3-1)*L3 + 9./2.*(3*L3 - 1)*L1;
	Hs[10] = -9*L1*L4-9./2.*(3*L1-1)*L4;
	Hs[11] = -9./2.*L1*L4 - 9./2.*(3*L4-1)*L4;
	Hs[12] = -9./2.*L2*L4;
	Hs[13] = -9./2.*L2*L4;
	Hs[14] = 9*L3*L4 + 9./2.*(3*L3-1)*L4;
	Hs[15] = -9./2.*L3*L4 + 9./2.*(3*L4 - 1)*L4;
	Hs[16] = -27*L2*L4;
	Hs[17] = 27*L2*L4;
	Hs[18] = -27*L3*L4+27*L1*L4;
	Hs[19] = -27*L2*L3 + 27*L1*L2;

	Ht[ 0] = -2*(3*L1-2)*L1 - 5./2.*(3*L1 - 1)*L1 - 0.5*(3*L1 - 1)*(3*L1 - 2);
	Ht[ 1] = -0.5*(3*L2 - 2)*L2 - (3*L2 - 1)*L2;
	Ht[ 2] = -0.5*(3*L3 - 2)*L3 - (3*L3-1)*L3;
	Ht[ 3] = (3*L4 - 2)*L4+ 0.5*(3*L4 - 1)*L4 + 0.5*(3*L4 - 1)*(3*L4 - 2);
	Ht[ 4] = -9*L1*L2 - 9./2.*(3*L1 - 1)*L2;
	Ht[ 5] = -9./2.*L1*L2 - 9./2.*(3*L2 - 1)*L2;
	Ht[ 6] = -9./2.*L2*L3;
	Ht[ 7] = -9./2.*L2*L3;
	Ht[ 8] = -9*L1*L3 - 9./2.*(3*L1 - 1)*L3;
	Ht[ 9] = -9./2.*L1*L3 - 9./2.*(3*L3 - 1)*L3;
	Ht[10] = -9*L1*L4 - 9./2.*(3*L1 - 1)*L4 + 9./2.*(3*L1 - 1)*L1;
	Ht[11] = 9*L1*L4 - 9./2.*(3*L4 - 1)*L4 + 9./2.*(3*L4 - 1)*L1;
	Ht[12] = -9./2.*L2*L4 + 9./2.*(3*L2 - 1)*L2;
	Ht[13] = 9*L2*L4 + 9./2.*(3*L4 - 1)*L2;
	Ht[14] = -9./2.*L3*L4 + 9./2.*(3*L3-1)*L3;
	Ht[15] = 9*L3*L4 + 9./2.*(3*L4 - 1)*L3;
...
//                          T E T 2 0 G 1 5
//=============================================================================
FETet20G15::FETet20G15() : FETet20_(NINT, FE_TET15G15)
{
	gr[0] = 0.25; gs[0] = 0.25; gt[0] = 0.25; gw[0] = 0.030283678097089;
...
//=============================================================================
//                          F E S H E L L Q U A D 8 E L E M E N T
//=============================================================================

void FEShellQuad8ElementTraits::init()
{
    int n;
...
//=============================================================================
//                          F E S H E L L T R I 6 E L E M E N T
//=============================================================================

void FEShellTri6ElementTraits::init()
{
    int n;
