...
void FEPenta15_::shape_deriv2(double* Hrr, double* Hss, double* Htt, double* Hrs, double* Hst, double* Hrt, double r, double s, double t)
{
    ...
    Htt[ 0] =(-1 + r + s)*(-1 + 2*r + 2*s);
    Htt[ 1] =r*(-1 + 2*r);
    Htt[ 2] =s*(-1 + 2*s);
    Htt[ 3] =(-1 + r + s)*(-1 + 2*r + 2*s);
    Htt[ 4] =r*(-1 + 2*r);
    Htt[ 5] =s*(-1 + 2*s);
    Htt[ 6] =-4*r*(-1 + r + s);
    Htt[ 7] =4*r*s;
    ...
    Hst[ 0] = ((-3 + 4*r + 4*s)*(-1 + 2*t))/2.;
    Hst[ 1] = 0;
    Hst[ 2] = ((-1 + 4*s)*(-1 + 2*t))/2.;
    Hst[ 3] = ((-3 + 4*r + 4*s)*(1 + 2*t))/2.;
    Hst[ 4] = 0;
    Hst[ 5] = ((-1 + 4*s)*(1 + 2*t))/2.;
    Hst[ 6] = r*(2 - 4*t);
    Hst[ 7] = 2*r*(-1 + 2*t);
    Hst[ 8] = -2*(-1 + r + 2*s)*(-1 + 2*t);
    Hst[ 9] = -2*(r + 2*r*t);
    Hst[10] = 2*(r + 2*r*t);
    Hst[11] = -2*(-1 + r + 2*s)*(1 + 2*t);
    Hst[12] = 2*t;
    Hst[13] = 0;
    Hst[14] = -2*t;

    Hrt[ 0] = ((-3 + 4*r + 4*s)*(-1 + 2*t))/2.;
    Hrt[ 1] = ((-1 + 4*r)*(-1 + 2*t))/2.;
    Hrt[ 2] = 0;
    Hrt[ 3] = ((-3 + 4*r + 4*s)*(1 + 2*t))/2.;
    Hrt[ 4] = ((-1 + 4*r)*(1 + 2*t))/2.;
    Hrt[ 5] = 0;
    Hrt[ 6] = -2*(-1 + 2*r + s)*(-1 + 2*t);
    Hrt[ 7] = 2*s*(-1 + 2*t);
    Hrt[ 8] = s*(2 - 4*t);
    Hrt[ 9] = -2*(-1 + 2*r + s)*(1 + 2*t);
    Hrt[10] = 2*(s + 2*s*t);
    Hrt[11] = -2*(s + 2*s*t);
    Hrt[12] = 2*t;
    Hrt[13] = -2*t;
...
FEPenta15G12::FEPenta15G12() : FEPenta15_(NINT, FE_PENTA15G12)
{
    const double ta = 1.0/3.0;
    const double tb = 1.0/5.0;
    const double tc = 3.0/5.0;
    const double wa = -27.0/96.0;
    const double wbc = 25.0/96.0;
    const double a = 0.774596669241483;
    const double w1 = 5.0 / 9.0;
    const double w2 = 8.0 / 9.0;
    gr[ 0] = ta; gs[ 0] = ta; gt[ 0] = -a; gw[ 0] = wa*w1;
    gr[ 1] = tc; gs[ 1] = tb; gt[ 1] = -a; gw[ 1] = wbc*w1;
    gr[ 2] = tb; gs[ 2] = tb; gt[ 2] = -a; gw[ 2] = wbc*w1;
    gr[ 3] = tb; gs[ 3] = tc; gt[ 3] = -a; gw[ 3] = wbc*w1;
    gr[ 4] = ta; gs[ 4] = ta; gt[ 4] =  0; gw[ 4] = wa*w2;
    gr[ 5] = tc; gs[ 5] = tb; gt[ 5] =  0; gw[ 5] = wbc*w2;
    gr[ 6] = tb; gs[ 6] = tb; gt[ 6] =  0; gw[ 6] = wbc*w2;
    gr[ 7] = tb; gs[ 7] = tc; gt[ 7] =  0; gw[ 7] = wbc*w2;
    gr[ 8] = ta; gs[ 8] = ta; gt[ 8] =  a; gw[ 8] = wa*w1;
    gr[ 9] = tc; gs[ 9] = tb; gt[ 9] =  a; gw[ 9] = wbc*w1;
    gr[10] = tb; gs[10] = tb; gt[10] =  a; gw[10] = wbc*w1;
    gr[11] = tb; gs[11] = tc; gt[11] =  a; gw[11] = wbc*w1;
    init();
}

//-----------------------------------------------------------------------------
//! \todo implement this
void FEPenta15G12::project_to_nodes(double* ai, double* ao)
{
