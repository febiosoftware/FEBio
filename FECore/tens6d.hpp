...
// operator +
inline tens6d tens6d::operator + (const tens6d& t) const
{
	tens6d s;
	for (int i=0; i<NNZ; i++)
		s.d[i] = d[i] + t.d[i];
...
// operator *
inline tens6d tens6d::operator * (double g) const
{
	tens6d s;
	for (int i=0; i<NNZ; i++)
		s.d[i] = g*d[i];
...
// assignment operator +=
inline tens6d& tens6d::operator += (const tens6d& t)
{
	for (int i=0; i<NNZ; i++)
		d[i] += t.d[i];
...
// assignment operator *=
inline tens6d& tens6d::operator *= (double g)
{
	for (int i=0; i<NNZ; i++)
		d[i] *= g;
...
// unary operator -
inline tens6d tens6d::operator - () const
{
	tens6d s;
	for (int i = 0; i < NNZ; i++)
		s.d[i] = -d[i];
...
// intialize to zero
inline void tens6d::zero()
{
	for (int i = 0; i < NNZ; i++)
		d[i] = 0;
}

inline void tens6d::unit()
{
	for (int i = 0; i < NNZ; i++)
		if ((i==0) || (i==171) || (i==323))
			d[i] = 1.;
...
inline tens3drs tens6d::contract3rs(tens3drs H)
{
    tens3drs K;
