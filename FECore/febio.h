...
template <typename T> class FERegisterClass_T : public FEBioFactory
{
public:
	FERegisterClass_T(SUPER_CLASS_ID sid, const char* sz) : FEBioFactory(sid, sz)
	{
...
{
	FEBioKernel& febio = FEBioKernel::GetInstance();
	return static_cast<TBase*>(febio.Create(sid, sztype, pfem));
}
