...
BEGIN_PARAMETER_LIST(FENewtonSolver2, FESolver)
...
//-----------------------------------------------------------------------------
FENewtonSolver2::FENewtonSolver2(FEModel* pfem) : FESolver(pfem)
{
    // default parameters
    m_LSmin = 0.01;
    m_LStol = 0.9;
...
//-----------------------------------------------------------------------------
FENewtonSolver2::~FENewtonSolver2()
{
...
//-----------------------------------------------------------------------------
FEGlobalMatrix& FENewtonSolver2::GetStiffnessMatrix()
{
    return *m_pK;
}
...
bool FENewtonSolver2::ReformStiffness(const FETimePoint& tp)
{
...
bool FENewtonSolver2::CreateStiffness(bool breset)
{
    // clean up the solver
...
//!           \
//!
bool FENewtonSolver2::InitEquations()
{
    // get the mesh
...
void FENewtonSolver2::Clean()
{
    if (m_plinsolve) m_plinsolve->Destroy();
}

//-----------------------------------------------------------------------------
void FENewtonSolver2::Serialize(DumpFile& ar)
{
...
bool FENewtonSolver2::SolveStep(double time)
{
    bool bret;
...
double FENewtonSolver2::LineSearch(double s)
{
    double smin = s;
