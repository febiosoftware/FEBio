...
//-----------------------------------------------------------------------------
void FESlidingSurface3::ShallowCopy(DumpStream& dmp, bool bsave)
{
...
//    #pragma omp parallel for shared(R, bupseg)
	for (int i=0; i<ss.Elements(); ++i)
	{
		...
		if (ssolu)
		{
			for (int j=0; j<ne; ++j) cs[j] = mesh.Node(el.m_node[j]).get(DOF_C + sid);
		}
			...
			if (pme)
			{
				...
				if ((Ln >= 0) && (g <= R))
				{
					...
					if (ssolu && msolu && (sid == mid)) {
						...
						for (int k=0; k<pme->Nodes(); ++k) cm[k] = mesh.Node(pme->m_node[k]).get(DOF_C + sid);
...
//-----------------------------------------------------------------------------
void FESlidingInterface3::ShallowCopy(DumpStream& dmp, bool bsave)
{
				...
				if (pme)
				{
					...
					if (tn > 0)
					{
						...
						if (ssolu && msolu && (sid == mid))
						{
							...
							for (k=0; k<nmeln; ++k) LM[k + nseln] = mLM[(4+mid)*nmeln+k];
...
void FESlidingInterface3::ContactStiffness(FESolver* psolver)
{
	...
	for (int np=0; np < npass; ++np)
	{
		...
		for (i=0; i<ss.Elements(); ++i)
		{
			...
			for (j=0; j<nseln; ++j)
			{
				pn[j] = ss.GetMesh()->Node(se.m_node[j]).get(DOF_P);
				cn[j] = ss.GetMesh()->Node(se.m_node[j]).get(DOF_C + sid);
			}
			...
			for (j=0; j<nint; ++j)
			{
				...
				if (pme)
				{
					...
					for (k=0; k<nmeln; ++k)
					{
						pm[k] = ms.GetMesh()->Node(me.m_node[k]).get(DOF_P);
						cm[k] = ms.GetMesh()->Node(me.m_node[k]).get(DOF_C + mid);
					}
